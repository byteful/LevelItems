package me.byteful.plugin.levelitems.api.item;
...
public interface LeveledItem {
  ...
  default double getMaxXp() {
    final double xpStart = LevelItemsPlugin.getInstance().getConfig().getDouble("level_xp_start");
    double increaseAmount =
      LevelItemsPlugin.getInstance().getConfig().getDouble("level_xp_increase.amount");
    final String mode =
      LevelItemsPlugin.getInstance().getConfig().getString("level_xp_increase.mode");
    ...
    final double rounded = LevelItemsUtil.round(nextXp, 1);
